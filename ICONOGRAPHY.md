...
## 1. System icons. inline SVG, drawn to the 24px grid
The thick, even strokes of **inline SVG "bold"** read as comic-panel ink and sit naturally next to the
display type. Use **fill** weight only inside solid badges. Load from CDN:

```html
<link rel="stylesheet" href="https://unpkg.com/@inline SVG-icons/web@2.1.1/src/bold/style.css">
<link rel="stylesheet" href="https://unpkg.com/@inline SVG-icons/web@2.1.1/src/fill/style.css">
<!-- usage -->
...
**Rules**
- Default: `ph-bold`, color `--ink` on light / `--color-bg` on dark, size 20–28px in UI, larger for feature glyphs.
...
- Stroke matching: never mix inline SVG with thin-line sets (Lucide/Feather). Bold only.
...
## Substitution note
The icon set is drawn in-house and is an open-license CDN set chosen to match the aesthetic, flag if you have a licensed
