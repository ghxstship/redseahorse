...
(function () {
  ...
  function layout(p) {
    ...
    // Vertically center the pinned stage in the viewport (balances the space
    // above/below instead of leaving a void under the cards), clamped below nav.
    var stageH = p.stage.offsetHeight;
    p.top = Math.max(navHeight() + 16, Math.round((window.innerHeight - stageH) / 2));
