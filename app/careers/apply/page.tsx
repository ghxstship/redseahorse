...
.cgrid{display:grid;grid-template-columns:1.3fr .7fr;gap:40px;padding:36px 0 0}@media(max-width:820px){.cgrid{grid-template-columns:1fr}}
  ...
  .field input,.field select,.field textarea{font-family:var(--font-body);font-size:var(--fs-body);background:var(--ink-2);border:var(--stroke-2) solid var(--ink-3);border-radius:var(--r-1);color:var(--bone);padding:var(--sp-3) 14px}
  ...
  .rail .blk{margin-bottom:22px}.rail .lbl{font-family:var(--font-mono);font-size:var(--fs-label);letter-spacing:.12em;text-transform:uppercase;color:var(--brass);margin:0 0 var(--sp-2)}
  .rail ol{margin:0;padding-left:18px}.rail li,.rail p{font-size:var(--fs-small);line-height:1.6;color:var(--fg-on-dark-2);margin:0 0 7px}
/*__nf__*/
...
<section className="pad"><div className="wrap"><div className="cgrid">
  <form>
    ...
    <div className="field"><label style={{display: 'flex', flexDirection: 'row', gap: '9px', alignItems: 'flex-start', fontFamily: 'var(--font-body)', fontSize: 'var(--fs-small)', letterSpacing: '0', textTransform: 'none', color: 'var(--fg-on-dark-2)'}}><input type="checkbox" style={{width: 'auto', marginTop: '3px'}}/>I&rsquo;m authorized to work in the US and consent to GHXSTSHIP storing this application.</label></div>
